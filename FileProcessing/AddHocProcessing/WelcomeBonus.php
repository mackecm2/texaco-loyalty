...
//		$endDate  =  $fields[$EndDateCol];
		...
		if( mysql_num_rows( $resultscard ) > 0 )
		{
			...
			else
			{
				...
				$sql = "Select * from PersonalCampaigns where MemberNo = $MemberNo and PromotionCode = 'WELCOME25'";
				...
				$sql = "Insert into CampaignHistory ( MemberNo, AccountNo, CampaignType, CampaignCode, CreationDate, CreatedBy ) values ( $MemberNo, $cardData[AccountNo], 'WELCOME', 'WEL01', '$startDate', 'LateBonus' )";
				...
				$sql = "Select * from Transactions where CardNo = '$cardNumber' and TransValue > 25 and TransTime between '$startDate 00:00:00' and Date_add('$startDate', interval 6 * 7 DAY ) order by TransTime limit 4";
				...
				$sql = "Insert into PersonalCampaigns (MemberNo, PromotionCode, StartDate, EndDate, PeriodSpend, PromoHitsLeft, CreationDate, CreatedBy ) values ( $MemberNo, 'WELCOME25', '$startDate', Date_add('$startDate', interval 6 * 7 DAY ), $SpendValue, $HitsLeft, now(), 'WelcProc' )";
				if( !mysql_query( $sql ))
				{	
					echo mysql_error() . $sql;
				}
