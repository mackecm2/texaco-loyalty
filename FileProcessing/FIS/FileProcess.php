...
//******************************************************************
//
// /FileProcessing/FIS/FileProcess.php
// Reads in a file from FIS and inserts the data into the database
//
...
//
//  	MRM 07/12/2010 - Original code based on Compower file process
//
//******************************************************************
...
$fileMove =  LocationFISFilesProcessed;
$filePattern = "STARREWARDS_TXNS*";
$fileToProcess = "";
$ProcessName   = "FIS";
...
function processHeaderLine( $line )
{
	...
	if( $len < 54 or $len > 56  )
	{
		...
		$mail->Subject = 'Corrupted FIS Transactions File - Please re-send'; // set mail subject
		...
		// HTML Message Body
		$mail->Body =
		'<font size=2>Ian<p>The FIS Transaction File '. $fileToProcess . '  cannot be read by our Star Rewards Batch process. Could you '."\n" 
		...
		// Text Message Body
		$mail->AltBody = "Ian: ".$fileToProcess."\n\n"
...
function TransactionInsert( $gUserData, $gSiteData, $fileNameOnly, $ProcessName )
{
	...
	$results = InsertTransaction( "FIS", $fileNameOnly );
...
function processTransaction( $line )
{
	...
	else
	{
		...
		if( checkForDuplicate( $cardNumber, $siteCode, "COMPOWER") && checkForDuplicate( $cardNumber, $siteCode, "FIS") )
		{
			// Check SiteNo
...
if( !$fileList )
{
	echo "\r\n";
	echo "************************************************************\r\n";
	echo "***POSSIBLE ERROR !! ***  No FIS Files found to process\r\n";
	echo "************************************************************\r\n";
	echo "\r\n";
	exit();
}
		...
		else 
		{
			...
			if ($goodSiteNumbers)
			{
				...
				if( processHeaderLine( $line ) )
				{
					...
					$InitialBalance = CreateExposurePoint( "Prior to FIS file load" .basename($fileToProcess) );
					...
					if( $fileRec )
					{
						...
						while( ( $line =  fgets( $fr ) )  && ( substr( $line, 0, 7 ) != "TRAILER" ) )
						{
							$lineNo++;
							//echo $line;
						...
						if ( $gStatsData->transactionsProcessed != $records OR $gStatsData->valueProcessed != $totalValue ) 
						{
							echo "\r\n";
							echo "************************************************************\r\n";
							echo "***POSSIBLE ERROR IN FIS FILE PROCESSING !! Mismatch in Summary Data *** r\n";
							echo "************************************************************\r\n";
							echo "\r\n";
						}
					}
					echo "Creating Exposure point\r\n";
					$FinalBalance = CreateExposurePoint( 'After FIS file load' .basename($fileToProcess) );
