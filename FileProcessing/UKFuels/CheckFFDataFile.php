...
error_reporting( E_ALL );
	...
	********************************************************************/
	...
	//******************************************************************
	//
	// /FileProcessing/UKFuels/CheckFFDataFile.php
	//
	// Checks for the monthly FFDATA.DAT file
	// sends an e-mail if no file found  
	// checks the size of the file
	// sends an e-mail if the file is empty
    //  
    //  MRM 14/12/2009 - First Issue
	//
	//******************************************************************
...
include "../../include/DB.inc";
	...
	function sendemail($fileToFind, $error)
	{
			...
			$mail->AddAddress('rebecca.stanley@ukfuels.co.uk', 'Rebecca Stanley'); // mail recipient address and name, repeat for each recipent
			$mail->AddAddress('mmackechnie@rsm2000.co.uk', 'Mike MacKechnie');

			$mail->AddCC('louise.buckingham@ukfuels.co.uk', 'Louise Buckingham'); 
			...
			{
			case 0:
			    $mail->Subject = 'Fast Fuels Data File (FFDATA.DAT) - File is empty';
			    $line2 = $fileToFind.' is empty. Please arrange for the file to be resent.</p></font>'."\n";
			    break;
			case 1:
			    $mail->Subject = 'Fast Fuels Data File (FFDATA.DAT) - File not found';
			    $line2 = $fileToFind.' has not been delivered. Please arrange for the file to be resent.</p></font>'."\n";
			    break;
			case 2:
			    $mail->Subject = 'Fast Fuels Data File (FFDATA.DAT) - File has the wrong number of columns';
			    $line2 = $fileToFind.' has the wrong number of columns. Please arrange for the file to be resent.</p></font>'."\n";
			    break;
			default: 
				$mail->Subject = 'Fast Fuels Data File (FFDATA.DAT) - Unknown Error';
			...
			// HTML Message Body
			$mail->Body =
			'<font size=2><font face=Verdana><p>The monthly check for the UK Fuels Fast Fuel Data File has found the following error: ' 
			.$line2
	...
	if (file_exists($fileToFind))
	{
		...
		fclose($handle);
   		...
   		$NumberOfColumns = ColumnCount($firstline) - 1; // not interested in the final (blank) column
