<?php
	...
	$sql = "select * from Analysis.Aug09StatementListcodes";
	...
	while( $row = mysql_fetch_assoc( $slaveRes ) )
	{
		...
		$sql = "Insert into Statement( AccountNo, StateDate, Balance, Mail_seg ) 
			values ( $row[AccountNo],'2009-08-24', $row[Balance], '$row[ListCode]')";
...
//		echo "$sql\n";
		...
		values ( $row[MemberNo], $row[AccountNo],'STATEMENT', 'AUG09', '$row[ListCode]', '2009-08-24 11:0:00', 'SteveT')";
