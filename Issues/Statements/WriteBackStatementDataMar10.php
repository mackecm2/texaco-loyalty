<?php
	...
	$sql = "select * from Analysis.Mar10StatementListcodes";
	...
	while( $row = mysql_fetch_assoc( $slaveRes ) )
	{
		...
		$sql = "Insert into Statement( AccountNo, StateDate, Balance, Mail_seg ) 
			values ( $row[AccountNo],'2010-03-10', $row[Balance], '$row[ListCode]')";
...
//		echo "$sql\n";
		...
		values ( $row[MemberNo], $row[AccountNo],'STATEMENT', 'MAR10', '$row[ListCode]', '2010-03-10 11:00:00', 'SteveT')";
