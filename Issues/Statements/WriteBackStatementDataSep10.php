<?php
	...
	$sql = "select * from Analysis.Sep10StatementListcodes";
	...
	while( $row = mysql_fetch_assoc( $slaveRes ) )
	{
		...
		$sql = "Insert into Statement( AccountNo, StateDate, Balance, Mail_seg ) 
			values ( $row[AccountNo],'2010-09-14', $row[Balance], '$row[ListCode]')";
...
//		echo "$sql\n";
		...
		values ( $row[MemberNo], $row[AccountNo],'STATEMENT', 'SEP10', '$row[ListCode]', '2010-09-14 15:00:00', 'SteveT')";
