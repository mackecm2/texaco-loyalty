<?php 
	...
	$AllSites = false;
	if( isset( $_GET["AllSites"] ) )
	{
		$AllSites = true;
		$sql = "Select SiteCode, AreaCode, RegionCode from sitedata order by SiteCode ";
	}
	else
	{
		$sql = "Select SiteCode, AreaCode, RegionCode from sitedata order by RegionCode, AreaCode ";
	}
	...
	while( $row = mysql_fetch_assoc( $results ))
	{
		if( !$AllSites )
		{
			if( $CurrentRegion != $row["RegionCode"] )
			{
				...
				echo $AreaExit;
				echo $RegionExit;
				echo "<TR><TD><IMG onClick=\"Toggle( this, T$unique)\" height=9 src=\"plus.gif\" width=9>\n<span onClick=\"PopulateRegionData('$row[RegionCode]')\">$CurrentRegion</span> <div id = T$unique  style=\"Display: none\">\n<Table >\n";
				$AreaExit = "";
			...
			if( $CurrentArea != $row["AreaCode"] )
			{
				...
				echo $AreaExit;
				echo "\t<TR><TD Width=10 ><TD ><IMG onClick=\"Toggle(this, T$unique)\" height=9 src=\"plus.gif\" width=9>\n<span onClick=\"PopulateAreaData('$row[RegionCode]','$row[AreaCode]' )\">$CurrentArea</span> \t<DIV id = T$unique style=\"Display: none\">\n\t <TABLE>\n";
				$AreaExit = "\t</TABLE></DIV>\n";
				$unique++;
				$CurrentArea = $row["AreaCode"];
			}
		}
		ECHO "\t\t<tr><td width=30><TD onclick=\"PopulateSiteData( '$row[RegionCode]', '$row[AreaCode]', '$row[SiteCode]')\">$row[SiteCode]</TR>\n";
	}
	ECHO $AreaExit;
	ECHO $RegionExit;
?>
