...
while ($data = fgetcsv ($handle, 2000, ","))
{
	...
	if($data[0] <> 'URN')
	{
		...
		if($row = mysql_fetch_assoc( $slaveRes ) )
		{
			...
			$updatesql = "update Members set 	Address1	= '$data[6]',
								Address2	= '$data[7]',		
								Address3	= '$data[8]',		
								Address4	= '$data[10]',		
								Address5	= '$data[11]',		
								PostCode	= '$data[12]'
					where MemberNo = '$row[MemberNo]' limit 1";
			...
			values ( $row[MemberNo], $row[AccountNo],'1', 'Address Change by Experian - Feb07 Statement run', now(), 'Steve')";
