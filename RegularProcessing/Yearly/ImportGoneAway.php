...
$handle = fopen ("/tmp/Goneaway.csv","r");
...
while ($data = fgetcsv ($handle, 2000, ","))
{
	...
	if($data[0] <> 'URN')
	{
		...
		if($row = mysql_fetch_assoc( $slaveRes ) )
		{
			...
			$updatesql = "update Members set GoneAway = 'Y' where MemberNo = '$row[MemberNo]' limit 1";
			...
			values ( $row[MemberNo], $row[AccountNo],'8', 'Marked GoneAway by Experian - Feb07 Statement run', now(), 'Steve')";
