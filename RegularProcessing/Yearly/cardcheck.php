...
$sql = "select CardNo,MemberNo,LastSwipeDate,CreationDate,CreatedBy from Cards where 1;";
...
while( $row = mysql_fetch_assoc( $slaveRes ) )
{
	...
	if(!validatecard($row['CardNo']))
	{
	
		// If the card is bad we need the other details
		
		echo"$row[CardNo],$row[MemberNo],$row[LastSwipeDate],$row[CreationDate],$row[CreatedBy]\n\r";
